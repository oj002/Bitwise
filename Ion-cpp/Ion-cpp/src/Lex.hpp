...
namespace Ion
{
	...
	static bool is_keyword_str(const char *str)
	{
	...
	static const char *token_kind_name(Token::Kind kind)
	{
		switch (kind)
		{
	...
	static const char *token_info()
	{
		if (token.kind == Token::NAME || token.kind == Token::KEYWORD) { return token.name; }
		else { return token_kind_name(token.kind); }
	}

	static inline uint8_t char_to_digit(char c) // TODO(Bernstein): in c++20 use the new c99 style Designated initializers
	{
		switch (c)
		{
	...
	static inline char escape_to_char(char c)
	{
		switch (c)
		{
		case 'n': return '\n';
	...
	static inline void init_stream(const char *str)
	{
		stream = str;
		next_token();
	}

	static inline bool is_token(Token::Kind kind) { return token.kind == kind; }

	static inline bool is_token_name(const char *name) { return token.kind == Token::NAME && token.name == name; }
	static inline bool is_keyword(const char *name) { return is_token(Token::KEYWORD) && token.name == name; }
	...
	static inline bool match_token(Token::Kind kind)
	{
		if (is_token(kind))
		{
			next_token();
	...
	static inline bool expect_token(Token::Kind kind)
	{
		if (is_token(kind))
		{
			next_token();
